...
//! - `token`: the source token the error must be attached to
...
enum Outcome {
    /// Expectation met.
    Pass,
    /// Error code met, but the fixture also pins an error `token` the engine
    /// did not attach. Reconciled against [`KNOWN_TOKEN_GAPS`] by (case,
    /// token), so both a new gap and a fixed one fail the suite.
    TokenGap {
        /// The token the fixture pins — the reconciliation key.
        token: String,
...
fn check_error(expected: &Expected, err: &JsonataError, stage: &str) -> Outcome {
    match expected {
        Expected::Error { code, token } => {
            ...
            match token {
                Some(want) if err.token != *want => Outcome::TokenGap {
                    token: want.clone(),
                    detail: format!(
                        "error {code}: expected token {want:?}, got {:?} ({})",
...
/// Cases whose error code is right but whose fixture `token` the engine
...
const EXPECTED_CASE_TOTAL: usize = 2469;
...
fn conformance_suite() {
    ...
    // (case name, expected token) for reconciliation, plus the detail line.
    let mut token_gaps: Vec<((String, String), String)> = Vec::new();
    ...
    for group in &groups {
        ...
        for case_entry in &cases {
            ...
            for tc in &test_cases {
                ...
                if let Outcome::TokenGap { token, detail } = &outcome {
                    token_gaps.push((
                        (case_name.clone(), token.clone()),
                        format!("{case_name}: {detail}"),
                    ));
                }
                match outcome {
                    Outcome::Pass | Outcome::TokenGap { .. } if expected_failure => {
                        passed += 1;
                        unexpected_passes.push(case_name.clone());
                    }
                    Outcome::Pass | Outcome::TokenGap { .. } => passed += 1,
    ...
    eprintln!(
        "Token gaps:         {} of {} fixture `token` expectations unmet (code correct)",
        token_gaps.len(),
        token_expectations,
    );
    ...
    if !token_gaps.is_empty() {
        eprintln!("\n── Token gaps (engine does not attach the pinned token) ──");
        for (_, detail) in &token_gaps {
    ...
    // Fixture `token` expectations: exactly the known gaps, no more, no
    // fewer. A new entry means the engine stopped attaching a token it used
    // to attach; a missing one means a gap was fixed and its KNOWN_TOKEN_GAPS
    // entry has to go with it.
    let mut observed: Vec<(String, String)> = token_gaps.into_iter().map(|(key, _)| key).collect();
    observed.sort();
    let mut known: Vec<(String, String)> = KNOWN_TOKEN_GAPS
        .iter()
        .map(|(case, token)| ((*case).to_string(), (*token).to_string()))
        .collect();
    known.sort();
    assert_eq!(
        observed, known,
        "unmet fixture `token` expectations changed — update KNOWN_TOKEN_GAPS \
         (an entry that disappeared is a fixed engine gap; a new one is a regression)"
    );
